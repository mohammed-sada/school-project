...
des.encrypt = function encrypt(message) {
  ...
  do {
    ...
    for (let i = 0; i < 48; i++) {
      //xor=(p.!q)+(!p.q)
      keyrightxor[i] =
        (parseInt(helpright.join("").charAt(i)) &&
          Number(!parseInt(Allkeys[Round].charAt(i)))) ||
        (Number(!parseInt(helpright.join("").charAt(i))) &&
          parseInt(Allkeys[Round].charAt(i)));
    }
    ...
    var finalfuncpermute = [
      16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14,
      32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25,
    ];
    var finalfuncpermutearr = new Array(32);
    for (let i = 0; i < 32; i++) {
      finalfuncpermutearr[i] = outbinary
        .join("")
        .charAt(finalfuncpermute[i] - 1);
    }
    ...
    for (let i = 0; i < 32; i++) {
      //xor=(p.!q)+(!p.q)
      NewRightplain[i] =
        (parseInt(finalfuncpermutearr.join("").charAt(i)) &&
          Number(!parseInt(Leftplain.join("").charAt(i)))) ||
        (Number(!parseInt(finalfuncpermutearr.join("").charAt(i))) &&
          parseInt(Leftplain.join("").charAt(i)));
    }
  ...
  //split the array into 8 places each one with 8 bits:
...
//   // Generate the key in the same way as it was done in the encrypt function
//   let keyword = "password";
//   var Extendedkey = new Array(keyword.length);
//   for (var i = 0; i < keyword.length; i++) {
//     // appending zero to the start make key length=64 as ascii =7bits only!
...
//   // Divide the key into two parts, Leftpart and Rightpart
//   var Rightpart = new Array(28);
//   var Leftpart = new Array(28);
//   for (let i = 0; i < 28; i++) {
//     Leftpart[i] = Shortenkey.join("").charAt(i);
...
//   // Split the encrypted message into two parts, Leftplain and Rightplain
//   let DESplain = encryptedMessage;
//   let Extendedplain = new Array(DESplain.length);
//   for (let i = 0; i < DESplain.length; i++) {
//     // appending zero to the start make plaintxt length=64 as ascii =7bits only!
//     Extendedplain[i] = 0 + "" + DESplain.charCodeAt(i.toString(2));
...
//   let Round = 0;
//   let Allkeys = new Array(16);
//   let Leftplains = new Array(16);
//   let Rightplains = new Array(16);

//   // Save the initial Leftplain and Rightplain in the arrays
//   Leftplains[Round] = Leftplain;
//   Rightplains[Round] = Rightplain;

//   // Loop for 16 rounds
//   do {
//     // Generate the key for this round in the same way as it was done in the encrypt function
//     let Numleftshifts = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];
//     let num = 0;
//     while (num < Numleftshifts[Round]) {
//       for (var i = 0; i < 28; i++) {
...
//       num++;
//     }
//     Allkeys[Round] = Leftpart.join("") + Rightpart.join("");
//     // Perform the round function in the same way as it was done in the encrypt function, but using the previous round's Leftplain and Rightplain as input
    
//     let temp = new Array(48);
//     for (let i = 0; i < 48; i++) {
//       temp[i] = Rightplains[Round].join("").charAt(Extendedkey[i] - 1);
//     }
//     let xorresult = new Array(48);
//     for (let i = 0; i < 48; i++) {
//       if (temp[i] == Allkeys[Round].charAt(i)) {
//         xorresult[i] = "0";
//       } else {
//         xorresult[i] = "1";
//       }
...
//     }
//     let index3 = 0;
//     let sboxoutput = new Array(8);
//     for (let i = 0; i < 8; i++) {
//       let row = xorresult[i * 6] + xorresult[i * 6 + 5];
//       let col =
//         xorresult[i * 6 + 1] +
//         xorresult[i * 6 + 2] +
//         xorresult[i * 6 + 3] +
...
//     var NewRightplain = new Array(32);
//     for (let i = 0; i < 32; i++) {
//       NewRightplain[i] = sboxoutput.join("").charAt(Finalpermute[i] - 1);
//     }
//     let temp2 = new Array(32);
//     for (let i = 0; i < 32; i++) {
//       if (NewRightplain[i] == Leftplains[Round].join("").charAt(i)) {
//         temp2[i] = "0";
//       } else {
//         temp2[i] = "1";
//       }
//     }
//     let NewLeftplain = temp2;
//     // Save the current round's Leftplain and Rightplain in the arrays
//     Leftplains[Round + 1] = NewLeftplain;
//     Rightplains[Round + 1] = Rightplains[Round];
//     Round++;
//   } while (Round < 16);
//   // After the 16th round, use the final Leftplain and Rightplain to generate the decrypted message
//   let inverseinitialpermute = [
//     40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14,
//     54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60,
//     28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41,
//     9, 49, 17, 57, 25,
//   ];
//   let finalpermute = new Array(64);
//   for (let i = 0; i < 64; i++) {
//     finalpermute[i] = Rightplains[Round].join("").charAt(
//       inverseinitialpermute[i] - 1
//     );
//   }
//   let finalplain = finalpermute.join("").slice(0, 8);
//   let result = String.fromCharCode(parseInt(finalplain, 2));
//   return result;
// };

const encryptedMessage = des.encrypt("belal");
// console.log(encryptedMessage);
// const d = des.decrypt(encryptedMessage);
// console.log(d);
